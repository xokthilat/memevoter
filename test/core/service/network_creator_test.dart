  ...
  group("network creator", () {
    test('should return success request', () async {
      ...
      dioAdapter.onGet(
        ...
        (server) => server.reply(200, fixture('listmemes_res.json')),
      );
      NetworkOptions networkOptions = NetworkOptions(
          onReceiveProgress: (int count, int total) => {});
      var response = await networkCreator.request(
          route: const ApiHolder.fetchListMeme(), options: networkOptions);
      expect(response.statusCode, 200);
      expect(response.data, fixture('listmemes_res.json'));
    });
  });
}
